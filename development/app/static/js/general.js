...
$(document).ready(function () {
    ...
    //need to fix styles in flow edit form
    if($('.js-notice-sender-email').length) {
        $('body').delegate("#sender_email_id", "focus blur", function() {
            var elem = $(this);
            setTimeout(function() {
                elem.parent().parent().find('.js-notice-sender-email').toggleClass("grey-text", elem.is( ":focus" ));
            }, 0 );
        });
    }

    $('body').on('shown.bs.dropdown', '.dropdown[data-container="body"]', function () {
        ...
        .on('hide.bs.dropdown', '.dropdown[data-container="body"]', function () {
            $dropdown = $(this);
            $dropdownMenu = $('body > .dropdown-menu');
            $dropdown.append($dropdownMenu.removeAttr('style').detach());
        });

    // (new notyPromotion('#notyFbVkTariffsBanner', 'notyFbVkTariffsBannerShowed', '')).show();
    (new notyPromotion('#notyVkReleaseBanner', 'notyVkReleaseBannerShowed', '')).show();

    if (window.localStorage) {
        if (localStorage.getItem('enteredMessengers') !== null) {
            (new notyPromotion('#notyFbVkTariffsBanner', 'notyFbVkTariffsBannerShowed', '')).show();
        }
    }

    // (new notyPromotion('#maintenense_2020010201', 'maintenense_2020010201', '')).show();
});

$(document).on('click', '.js-user-dropdown-menu', function (e) {
    e.stopPropagation();
});
...
$("body").on("click", ".pagination a, .pagination-page a", function () {
    ...
    } else if (className == 'pagination-scheduled') {
        refreshContentPaginationScheduledTasks($(this).attr("href"));
    }
...
function setCursorToTextField(sBlockName) { // Feature #12775
    ...
    if (window.location.pathname.indexOf('/emailservice/other-settings') != -1) {
        return false;
    }
    ...
    if ($(sBlockName + ' input[type=text], ' + sBlockName + ' input[type=email], ' + sBlockName + ' input[type=password], ' + sBlockName + ' input[type=number], ' + sBlockName + ' textarea').length > 0) {
        ...
        if (!bSetFocus && $(sBlockName + ' input[type=text]').length) {
            $(sBlockName + ' input[type=text]').each(function () {
                if (!$(this).hasClass('no-set-focus') && !$(this).hasClass('datepicker_input')) {
...
$('#tab-bookmarks li a, div.bookmark-arrow, #search-filter a, .need-set-focus a, .set-focus-after-click').on("click", function () { // переход по вкладкам, фильтр
    if($('#tab-bookmarks') && (this.dataset.tabname != undefined)){
        window.location.hash = this.dataset.tabname;
    }
...
$(function () {
    ...
    refreshJsPagination();
    $('body').on('click', '#search-filter .reset', function (e) {
...
$(document).ready(function () {
    ...
    $('#service-menu-link').on('click', function () {
        $('.js-navbar-item:not(.active)').toggleClass('hidden-item');
    ...
    $(window).on('refferalPopoverShow', function(e, data){
        if ($('#refferal_nav_banner_link').length){
            $('#refferal_nav_banner_link').focus()
                .popover('show')
                .data('bs.popover')
                .tip()
                .addClass('popover-refferal');
        }
        if (window.localStorage) {
...
function setActiveMenuItem() {
    ...
    $('#menu-navbar').find('li.js-navbar-item a').reverseEach(function () {
...
function refreshContentPaginationScheduledTasks(url, onPage) {
    $('#tasks-scheduled-info').block();
    url = url.replace('/members/emailservice/tasks/index', '/members/emailservice/tasks/get-scheduled-tasks');

    if (onPage !== undefined) {
        url = url.replace(/\/onPage\/\d+/gi, '/');
        if (url[url.length-1] == '/') {
            url += 'onPage/' + onPage;
        } else {
            url += '/onPage/' + onPage;
    ...
    $.get(
        ...
        function (data) {
            //console.log(data);
            $('#tasks-scheduled-info').html(data);
            $('#tasks-scheduled-info').unblock();
        })
}
...
function refreshFilterParametrsBox() {
    ...
    var sParamBlank = '<span class="one-filter-params" data-filtername="{{FILTER_NAME}}" data-filtervalue="{{FILTER_VALUE}}">{{NAME}}: <span>{{VAL}}</span> {{REMOVE_BLOCK}}</span>';
    ...
    $('#search-filter form').find('.send-val:not([type=hidden])').each(function (index, el) {
        var lt = /</g,
            gt = />/g;
        el.value = el.value.toString().replace(lt, "&lt;").replace(gt, "&gt;");
        if (el.value || $(this).data('defaultval')) {
            ...
            if (($(this).hasClass('datepicker_input')) || $(this).attr('name') === 'date1' || $(this).attr('name') === 'date2') {
                var sCurrentLang = ($.inArray($('#current_lang').text(), ['en', 'ua', 'ru', 'es-mx', 'pt-br', 'tr']) !== -1) ? $('#current_lang').text() : 'en';
            ...
            sResult = sResult.replace('{{FILTER_NAME}}', el.name);
            sResult = sResult.replace('{{FILTER_VALUE}}', el.value);
        }
    });
    $('#search-filter .one-filter-params').remove();
...
function resetFilterValue(oParamLabel, sElementName, sMultiSelectVal) {
    if (oParamLabel && sElementName) {
        ...
        if (typeof myTasks !== 'undefined' && $.isFunction(myTasks.showPeriodStatBlock)) {
            myTasks.showPeriodStatBlock();
        }
...
function filterChangeDates(from, to) {
    $('#order-date-from')
        .val(from)
        .data('defaultval', from);
...
function sendQuery(obj, get_url_params) {
    ...
    $(obj).find('.send-val-radio').each(function (index, el) {
        var $radio = $(el);
        if($radio.prop('checked') && $radio.val() ){
            str += el.name + '/' + $radio.val() + '/';
        }
    });
...
function checkform(obj) {
    ...
    var validation = $(obj).find("#message_title.valid, .valid:visible:enabled").not('.disabled');
    ...
    if (validation) {
        ...
        validation.each(function (index, el) {
            ...
            if ($(this).hasClass('email')) {
                ...
                if (!$.trim(el.value) && !$.trim(el.innerText) ) {
                    error = true;
                    $(this).parent().addClass('has-error');
                }
            }
...
// Allow: backspace, delete, tab, escape, space, +, enter and .
        ...
        function getContract(obj) {
            ...
            if (readCookie('agreement_readed'))
                ...
                location.replace("/orders/get-document/type/5/");
            else {
...
// QUEST HOOK
        ...
        function sendActionOrder(c_orderid, type) {
            if( typeof isDownloadDocInExcelFormat === 'undefined' ){
                isDownloadDocInExcelFormat = 0;
            }else {
                isDownloadDocInExcelFormat = isDownloadDocInExcelFormat || 0;
            }
            if (!type || type == 0) {
                type = '200';
            }
            if (!c_orderid)
                return false;
            if(type == '30' || type == '40'){
                if(isDownloadDocInExcelFormat) {
                    downloadDocInExcelFormat(c_orderid);
                    return true;
                }
                var actionUrl = '/products/bank/';
                if(type == '30'){
                    actionUrl = '/products/law/';
                }
                $.ajax({
                    url: actionUrl,
                    type: 'get',
                    data: { 'orders': c_orderid[0],'force_pdf':true},
                    cache: false,
                    dataType: 'json',
                    success: function( response ) {
                        $.unblockUI();
                        $('#billSuccessModal').modal('show');
                        exportIntoPdf(response.pdf_hash);
                        $('#download_pdf_bill').on('click', function(){
                            exportIntoPdf(response.pdf_hash);
                        });
                    }
                });
            ...
            if(type == '220'){
                var cur = prlist['payment_currency'];
                var sum = parseFloat(prlist['sum']);
                if(!cur && typeof payment_currency != 'undefined' && payment_currency!=null) {
                    cur = payment_currency;
                }
                var email = $('#payer_email').html();
                var description = '';
                if ($('#selecetProductName').length && $('#selecetProductName').html().length>0){
                    description = $('#selecetProductName').html();
                }
                CloudPaymentspay(c_orderid[0],cur,sum,email,description);
                return true;
            }
            ...
            if (type == '100')
            ...
            else if (type == '200' || type == '210' || type == '230')
                ...
                if($('#liqpay-subscription').length) {
                    form.append($('#liqpay-subscription'));
                }
        ...
        function merciPayment(service,abbr,monthes,additional_amount) {
            if(service !== 'undefined' && abbr!== 'undefined' && monthes!== 'undefined' ) {
                $.blockUI();
                $.ajax({
                    url: "/loyalty-program-ajax/buy-tariff/",
                    cache: false,
                    type: 'post',
                    data: {
                        "service": service,
                    ...
                    success: function (data) {
                        if(data.result) {
                            redirectUrl = '/emailservice/';
                            if (service != 'email') {
                                redirectUrl = '/' + service + '/';
                            }
                            window.location.href = redirectUrl+'?order=complete';
                        }
                        $.unblockUI();
                    },
        ...
        function CloudPaymentspay(orderId, cur, sum, email, description) {
            var language = "ru-RU";
            ...
            if(!cur || sum <=0 || email == '') return false;
            var redirectUrl = '/emailservice/';
            if ($('#module-name').length && $('#module-name').html() != '') {
                redirectUrl = '/' + $('#module-name').html() + '/';
            }
            var widget = new cp.CloudPayments({language: language, googlePaySupport: false});
            widget.charge({ // options
                    publicId: $('#cloudPaymentspk').html(),
...
var a = {
    ...
    "İ": "i"
        ...
        function getAddressesInfo(iAddressBookID, sEmailAddresses, base64Email) {
            var dataToSend = {
            ...
            $.post('/emailservice/addressbooks/get-id-by-address/', dataToSend, function (response) {
                ...
                if (response != -1) {
                    ...
                    window.location.href = '/emailservice/addressbooks/emailview/email/' + base64Email;
                }
            }).fail(function(r) {
                $.unblockUI();
                new Noty({
        ...
        function check_url(url) {
            /*
             ...
             // Same in application/modules/members/modules/emailservice/model/RequestsFreeTariff.php
...
function addNewSenderEmailAddress(){
        ...
        var dataToSend = {
            ...
            sender_domain: $('#add_sender_domain_selection').val(),
            sender_email_privacy_url: $('#new_sender_privacy_url').val(),
            sender_email_company: $('#new_sender_company').val()
        };
        $.blockUI();
        $.ajax({
            ...
            success: function( response ) {
                ...
                $.ajax({
                            ...
                            if(typeof (response.isfree) != 'undefined' && response.isfree==1){
                                BootstrapDialog.show({
                                    ...
                                    buttons: [{
                                        ...
                                        action: function (dialog) {
                                            dialog.close();
                                            window.location.href = '/emailservice/billing/tariffs/?at_su=def';
                                        }
                                    }]
                                });
                            }else{
                    ...
                    showAlert(response.comment, '', 'large-modal');
                }
            }
    })
}
...
function initTabHashUrl($tabs) {
    if( $tabs &&  $($tabs).length ) {
        ...
        $($tabs +' a').on('shown.bs.tab', function (e) {
            ...
            window.scrollTo(0, 0);
            $isUrlHashSeted = false;
        });
        $(window).on('hashchange', function() {
            if (!document.location.hash) {
                $($tabs).find('li').first().find('a').tab('show');
            } else {
...
function setclose_top_alert_wrapper(cookieName) {
    $.ajax({
        url: '/settings/set-close-top-alert-wrapper-cookie',
        dataType: "text",
        type: 'POST',
    ...
    document.cookie = "close_top_alert_wrapper=1";
    document.cookie = "expires=" + now.toUTCString() + ";"
}

function getTranslations(service,sections){
    $.ajax( {
        url     : "/localization/get-section/",
        cache   : false,
        type    : 'post',
        data    : {
            "service": service,
...
function downloadDocInExcelFormat(orderId) {
    var type = $( '.products-pmselect input[name=pay_method]:checked' ).val();
    var href="/orders/get-document/id/" + orderId + "/type/";
    href += (40 == type) ? "4/" : "2/";

    //30 - счет для юрлица, 40 - квитанция в банк для физлица

    $.unblockUI();
    $('#billSuccessModal').modal('show');
    $('#download_pdf_bill_link').removeClass('hidden');

    $('#download_pdf_bill').unbind('click');
    $('#download_pdf_bill').on('click', function(){
...
function Currency() {
    ...
    self.CURRENCY_CONFIG = {
        TWO_DIGITS_CURRENCIES: [CUR_UAH, CUR_RUB, CUR_KZT],
        RU_ZONE_LANGUAGES: [LANGUAGE_UA, LANGUAGE_RU],
        ADD_TRAILING_ZEROES: false,
    };

    self.formatPrice = function (price, currency, language) {
        var userCurrency = currency || $('#user_currency').text();
        var userLanguage = language || $('#current_lang').text();

        if(userCurrency === CUR_CLP) {
            price = Math.round(+price);
        }

        var decimals = self.getDecimalsCount(userCurrency, userLanguage);
        var decPoint = self.getDecimalPointSeparator(userCurrency, userLanguage);
        var thousandsSep = self.getThousandsSeparator(userCurrency, userLanguage);
        ...
        return self.number_format(
            ...
            decPoint,
            thousandsSep,
            self.CURRENCY_CONFIG.ADD_TRAILING_ZEROES
        );
    }

    self.getDecimalsCount = function (currency, currentLanguage) {
        if (~$.inArray(currentLanguage, self.CURRENCY_CONFIG.RU_ZONE_LANGUAGES)) {
            return 2;
        }
    ...
    self.getDecimalPointSeparator = function (currency, currentLanguage) {
        if ((~$.inArray(currentLanguage, self.CURRENCY_CONFIG.RU_ZONE_LANGUAGES)) || currency == CUR_BRL) {
            return ',';
        }
    ...
    self.getThousandsSeparator = function (currency, currentLanguage) {
        if (~$.inArray(currentLanguage, self.CURRENCY_CONFIG.RU_ZONE_LANGUAGES)) {
            return ' ';
        }

        if (~$.inArray(currency, [CUR_BRL, CUR_CLP])) {
            return '.';
        }
    ...
    self.number_format = function (number, decimals, dec_point, thousands_sep, add_trailing_zeroes) {
        ...
        if ((s[1] || '').length < prec && add_trailing_zeroes) {
            s[1] = s[1] || '';
...
var notyPromotion = function (contentElem, chacheVariableName, additionalClassName) {
    var self = this;
    this.show = function() {
        if ($(contentElem).length && !this.isShowed()) {
            setTimeout(function() {
                new Noty({
                    type: 'info',
                    theme: 'bootstrap-v3 noty-promotion ' + additionalClassName,
                    layout: 'bottomRight',
                    text: $(contentElem).html(),
                    closeWith: ['click', 'button']
                }).show().on('onClose', function () {
                    self.setShowed();
                });
            }, 3000);
        }
    ...
    this.isShowed = function() {
        if (window.localStorage) {
            return !!parseInt(localStorage.getItem(chacheVariableName));
        }
        return false;
    };

    this.setShowed = function() {
        if (window.localStorage) {
            localStorage.setItem(chacheVariableName, 1);
        }
    }
};
